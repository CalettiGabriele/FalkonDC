...
class Falkon(FalkonBase):
    ...
    def __init__(
        ...
        center_selection: Union[
            str, falkon.center_selection.CenterSelector
        ] = "uniform",
        maxiter: int = 20,
        seed: Optional[int] = None,
        error_fn: Optional[
            Callable[[torch.Tensor, torch.Tensor], Union[float, Tuple[float, str]]]
        ] = None,
        ...
        super().__init__(
            kernel, M, center_selection, seed, error_fn, error_every, options
        )
    ...
    def fit(
        ...
        with torch.autograd.inference_mode():
            ...
            if self.weight_fn is not None:
                ...
                ny_points, ny_indices = self.center_selection.select_indices(torch.load("/root/Data/X_train.pt"), None)
            else:
                #ny_points: Union[torch.Tensor, falkon.sparse.SparseTensor] = self.center_selection.select(X, None)
                ny_points: Union[torch.Tensor, falkon.sparse.SparseTensor] = self.center_selection.select(torch.load("/root/Data/X_train.pt"), None)
                ny_indices = None
            ...
            _use_cuda_mmv = (
                self.use_cuda_
                and tot_mmv_mem_usage / self.num_gpus
            ...
            with TicToc(
                f"Calcuating Preconditioner of size {num_centers}",
                ...
                ny_weight_vec = None
                if self.weight_fn is not None:
                    ny_weight_vec = self.weight_fn(
                        Y[ny_indices], X[ny_indices], ny_indices
                    )
            ...
            if self.error_fn is not None and self.error_every is not None:
                validation_cback = self._get_callback_fn(
                    X, Y, Xts, Yts, ny_points, precond
                )
    ...
    def _predict(self, X, ny_points, alpha: torch.Tensor) -> torch.Tensor:
        with torch.autograd.inference_mode():
            ...
            _use_cuda_mmv = alpha.device.type == "cuda" or (
                self.use_cuda_
                and tot_mmv_mem_usage / self.num_gpus
                >= get_min_cuda_mmv_size(X.dtype, self.options)
            )
